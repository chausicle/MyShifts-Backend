  ...
  const msg = {
    ...
    html: `<html>
      ...
      <body>
        ...
        <div id="small">If this is incorrect, please go back to your <a href="http://myshifts.us" target="blank">myShifts</a> app and make the necessary changes.</div>
...
const processShiftEmail = (employeeId, shiftId, userId, start, date) => {
  sgMail.setApiKey(process.env.SENDGRID_API_KEY);
  console.log(process.env.SENDGRID_API_KEY);
  ...
  .then(result => {
   const email = result[0].email
   sendShiftEmail(email, start, date)
 })
}

const sendShiftEmail = (email, start, date) => {
  console.log(email);
  const msg = {
    to: `${email}`,
    from: 'updates@myshifts.us',
